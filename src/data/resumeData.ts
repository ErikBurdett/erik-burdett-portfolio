export const personalInfo = {
  name: "Erik Burdett",
  title: "Full Stack Developer",
  subtitle: "QA - CI/CD - DevOps - Technical Writer",
  location: "Amarillo, Texas",
  phone: "806-677-4249",
  email: "erikaburdet@gmail.com",
  github: "https://github.com/ErikBurdett",
  linkedin: "https://linkedin.com/in/erikburdett",
  resumePdf: "/Erik-Burdett-Resume-2025.pdf",
};

export const skills = {
  technical: [
    "React", "Next.js", "Node.js", "Express", "PHP",
    "Python", "TypeScript", "JavaScript",
    "GraphQL", "REST APIs",
    "AWS", "Azure", "Docker",
    "GitHub Actions", "CI/CD",
    "PostgreSQL", "SQL Server",
    "Linux Systems Admin",
    "Jest", "Pytest", "Selenium",
    "WordPress", "Custom WordPress Themes",
    "WordPress Plugin Development", "Custom PHP Development",
    "Microservices", "Agile/Scrum"
  ],
  categories: {
    frontend: ["React", "Next.js", "TypeScript", "JavaScript", "Vite", "MUI"],
    backend: ["Node.js", "Express", "Python", "Flask", "PHP", "GraphQL", "REST APIs", "Microservices"],
    devops: ["AWS", "Azure", "Docker", "GitHub Actions", "CI/CD", "Terraform", "Linux", "WHM cPanel"],
    database: ["PostgreSQL", "SQL Server", "Snowflake"],
    testing: ["Jest", "Pytest", "Selenium", "Preflight", "Rainforest QA", "Automated UI Testing", "API Testing"],
    tools: ["Git", "Jira", "Helpscout", "WordPress"],
    wordpress: ["WordPress", "Custom Themes", "Plugin Development", "Custom PHP", "WooCommerce", "WordPress Integrations"],
  },
  content: [
    "Photography", "Videography", "Design/Wireframes",
    "Video Editing", "Photo Editing", "Content Management",
    "Copywriting", "Technical Documentation"
  ]
};

export const experience = [
  {
    id: 0,
    company: "Dharma Web Solutions",
    role: "Owner / Lead Full Stack Developer",
    period: "December 2024 - Present",
    type: "Full-time",
    technologies: ["React", "Next.js", "Node.js", "Python", "TypeScript", "AWS", "WordPress", "Custom PHP", "Plugin Development"],
    highlights: [
      "Founded and operate a full-service web development consultancy specializing in custom web applications and WordPress solutions",
      "Deliver end-to-end development services including custom React/Node applications, WordPress theme and plugin development, and cloud deployments",
      "Provide DevOps consulting, CI/CD pipeline setup, and infrastructure optimization for small to mid-sized businesses",
      "Specialize in migrating legacy systems to modern tech stacks while maintaining business continuity",
      "Offer technical consulting, code audits, and architecture planning for clients across various industries"
    ]
  },
  {
    id: 1,
    company: "Andrews & Associates L.T. Solutions",
    role: "Fullstack Developer",
    period: "September 2024 - Present",
    type: "Full-time",
    technologies: ["React", "Node", "Python", "AWS", "Azure", "GitHub Actions", "CI/CD", "WordPress", "Linux"],
    highlights: [
      "Designed modern full stack architecture to replace legacy WordPress ecosystem with scalable React, Node, and Python services",
      "Rebuilt and optimized WordPress legacy systems, WHM cPanel, and Linux-based infrastructure to reduce technical debt and improve maintainability",
      "Built internal tooling and automated workflows using CI/CD, AWS, Azure, and GitHub Actions",
      "Implemented QA standards, documentation practices, and team development methodologies",
      "Mentored distributed teams and coordinated cross-department collaboration between IT support and development",
      "Came on as the company's first web developer and helped build a modern development team to alleviate IT support and field technicians of development duties",
      "Data analysis and utility scripting with Python, Flask, Node, and React"
    ]
  },
  {
    id: 2,
    company: "KTBlack Services",
    role: "Lead Fullstack Developer/QA",
    period: "October 2022 - September 2023",
    type: "Full-time",
    technologies: ["React", "Vite", "TypeScript", "MUI", "Node", "AWS EC2", "Salesforce", "Angular", "WebSockets"],
    highlights: [
      "Architected and developed full stack application to replace legacy Salesforce solution",
      "Built frontend and backend with React (Vite, TypeScript, MUI), Node, AWS EC2",
      "Developed Angular messaging system using WebSockets within Salesforce",
      "Established DevOps norms and modern engineering practices",
      "Set up QA methodologies, initial CI/CD and automated testing systems",
      "Worked with offshore remote development team for fast-tracking crucial UI development",
      "Mentored and taught junior developers in Node.js and React"
    ]
  },
  {
    id: 3,
    company: "Audiohook",
    role: "Fullstack Engineer/QA Lead",
    period: "September 2021 - May 2022",
    type: "Remote",
    technologies: ["Python", "Flask", "React", "TypeScript", "GraphQL", "AWS", "Terraform", "Jest", "Pytest"],
    highlights: [
      "Created CI/CD testing pipeline & Test Driven Development practices with CircleCI, then GitActions",
      "Developed and maintained features across Python Flask API, GraphQL, and React frontend",
      "UI & QA testing through Preflight and Rainforest QA, plus Jest unit tests",
      "API QA & Testing with Python Flask, Snowflake, SQL, and Pytest",
      "Wrote documentation for bug fixes, onboarding, feature documentation, and DevOps practices"
    ]
  },
  {
    id: 4,
    company: "CreativePickle",
    role: "Summer Intern",
    period: "May 2015 - September 2015",
    type: "Internship + Contract",
    technologies: ["WordPress", "Photography", "Videography", "Copywriting"],
    highlights: [
      "Provided video and photography services, copywriting, and WordPress media management",
      "Occasional contract work continuing to present day"
    ]
  }
];

export const freelanceExperience = [
  {
    id: 1,
    company: "EightyTwenty",
    role: "Lead Full Stack Developer",
    period: "July 2019 - Present",
    highlights: [
      "Migrated business site from Squarespace → WordPress → fully custom React/Node platform with online ordering and POS integration",
      "Integrated Toast POS, Square, Stripe, PayPal, Venmo, and Cash App for unified payment processing",
      "Built automated workflows linking Google Sheets/Forms to backend systems"
    ]
  },
  {
    id: 2,
    company: "Randall County Rentals",
    role: "WordPress Developer",
    period: "January 2019 - February 2022",
    highlights: [
      "Maintained and expanded property management site",
      "Created photo and video content",
      "Integrated Real Estate Management plugin with mailing service and appointment reminders"
    ]
  },
  {
    id: 3,
    company: "Saint Francis De Sales School for The Deaf",
    role: "WordPress Developer & Content Creator",
    period: "December 2016 - April 2023",
    location: "Nyapea, Uganda",
    highlights: [
      "Traveled to Uganda with Founder Rannah Evetts to help open the 90-student school",
      "Created social media content (photo & video) and managed social media presence",
      "Built volunteer and donation portal increasing donations by $20,000+ in 2017 and $45,000 in 2022"
    ]
  }
];

export const projects = [
  {
    id: 1,
    title: "Family Promise Service Tracker",
    role: "Full Stack Developer",
    description: "A full stack React/Node application used to track and visualize services provided to families. Improved frontend UX, API integrations, and data visualization layers displaying cost metrics and case notes across an interactive, mobile-friendly map.",
    technologies: ["React", "Node.js", "Express", "PostgreSQL"],
    github: "https://github.com/Lambda-School-Labs/family-promise-service-tracker-be-a",
    type: "Open Source"
  },
  {
    id: 2,
    title: "EightyTwenty Platform",
    role: "Lead Full Stack Developer",
    description: "Migrated business site from Squarespace → WordPress → fully custom React/Node platform with online ordering and POS integration. Integrated Toast POS, Square, Stripe, PayPal, Venmo, and Cash App for unified payment processing. Built automated workflows linking Google Sheets/Forms to backend systems.",
    technologies: ["React", "Node.js", "Toast POS", "Stripe", "Square", "PayPal", "Google Sheets API"],
    type: "Freelance"
  },
  {
    id: 3,
    title: "KTBlack Services Platform",
    role: "Lead Fullstack Developer/QA",
    description: "Architected and developed a full stack application to replace the KTBlack Legacy Salesforce solution. Built frontend and backend with React (Vite, TypeScript, MUI), Node, AWS EC2. Developed an Angular messaging system using WebSockets within Salesforce for internal and external communication. Established DevOps norms and modern engineering practices including version control, agile development, and scrum.",
    technologies: ["React", "TypeScript", "MUI", "Vite", "Node.js", "AWS EC2", "WebSockets", "Angular", "Salesforce"],
    type: "Enterprise"
  },
  {
    id: 4,
    title: "Saint Francis Ministries",
    role: "WordPress Developer & Content Creator",
    description: "Created the Saint Francis Ministries website for volunteer and donation portal for a school for the deaf in Nyapea, Uganda. The platform increased donations by over $20,000 in 2017 and $45,000 in 2022. Traveled to Uganda with Founder Rannah Evetts, helped with day-to-day operations, and created social media content.",
    technologies: ["WordPress", "JavaScript", "PHP", "Custom Themes"],
    type: "Non-Profit"
  },
  {
    id: 5,
    title: "Audiohook Platform",
    role: "Fullstack Engineer/QA Lead",
    description: "Developed end-to-end features across the Python Flask API, GraphQL, and React frontend. Created Audiohook's CI/CD testing pipeline and Test Driven Development practices, transitioning from CircleCI to GitHub Actions with AWS & Terraform deployments. Led UI & QA testing through Preflight and Rainforest QA, wrote and maintained Jest unit tests for frontend, and built automated API testing with Pytest for the Flask API and Snowflake integrations.",
    technologies: ["Python", "Flask", "React", "TypeScript", "GraphQL", "AWS", "Terraform", "GitHub Actions", "Jest", "Pytest", "Snowflake", "Automated Testing"],
    type: "Enterprise"
  },
  {
    id: 6,
    title: "Randall County Rentals",
    role: "WordPress Developer",
    description: "Maintained and expanded property management site, created media content, and integrated real estate WordPress tools. Integrated Real Estate Management plugin for WordPress with mailing service and appointment reminders. Created photography and videography content for property listings.",
    technologies: ["WordPress", "PHP", "JavaScript", "Real Estate Plugins", "Photography", "Videography"],
    type: "Freelance"
  },
  {
    id: 7,
    title: "Andrews & Associates WordPress Phoenix Project",
    role: "Fullstack Developer / Systems Architect",
    description: "Designed a modern full stack architecture to replace a legacy WordPress ecosystem, emphasizing scalable React, Node, and Python services. Rebuilt and optimized WordPress legacy systems, WHM cPanel, and Linux-based infrastructure to reduce technical debt and improve maintainability. Built internal tooling and automated workflows using CI/CD, AWS, Azure, and GitHub Actions.",
    technologies: ["React", "Node.js", "Python", "WordPress", "AWS", "Azure", "GitHub Actions", "CI/CD", "Linux", "WHM cPanel"],
    type: "Enterprise"
  }
];

export const education = [
  {
    institution: "Bloom Institute of Technology",
    degree: "Full Stack Development",
    period: "September 2021"
  },
  {
    institution: "West Texas A&M University",
    degree: "English & Computer Science",
    period: "2014 - 2019"
  },
  {
    institution: "University of Colorado",
    degree: "English & Computer Science",
    period: "2013 - 2014"
  }
];

